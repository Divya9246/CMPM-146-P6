...
- Jenny / YHZ (`yzhen174`) — dog-vs-cat transfer model + random model (section 8)
...
We built a facial emotion classifier (neutral / happy / surprise), hooked it up so a webcam can play tic-tac-toe by making faces, and then Jenny reused the CNN backbone for a dog-vs-cat transfer learning experiment (plus a random-weight control).
...
**Transfer learning (Jenny’s part)**  
...
1. Divya sent the P6 writeup / plan; Jenny said it looked good and shared GitHub (`yzhen174`). Invite went out, repo shared.
2. Divya used FER-2013; Jenny later picked the dog-vs-cat set for transfer.
3. Divya finished the CNN + training + webcam game first (~74.7% at that point), pushed, and Jenny pulled once the PR was actually merged (classic “empty pull” scare — it was just the PR not merged yet).
4. Jenny finished transfer + random models, noted transfer looked stronger early (first few epochs) and random caught up later overall — but transfer still had the early val accuracy bump the assignment cares about. Jenny also saw ~64% when retraining the basic model on their machine (about 10% off Divya’s number), which matters because transfer/random load that base model.
...
6. Jenny later tweaked transfer/random (epochs / dense size) when a run didn’t meet requirements, and shared their dog-vs-cat PDF report. Divya’s focus stayed on sections 5–7; Jenny owns the section 8 writeup / numbers.
...
## Jenny’s part (section 8) — transfer + random (summary)

Jenny owns the details / PDF numbers; this is the high-level version from our chat and the code in the repo.
...
- When Jenny tested: transfer had higher accuracy for the first few epochs; later random could pull ahead overall. Transfer still showed the early validation accuracy the assignment wants after epoch 1
- Jenny recommended Divya also verify transfer on their machine, because the base model accuracy can differ by ~10% depending who trains it
- Jenny later changed epochs / dense size when a run didn’t meet requirements, and shared `dog vs cat transfer model report.pdf`
...
(Jenny’s scripts expect the dog-vs-cat data laid out the way `preprocess.py` / `train_transfer.py` describe.)
...
- Jenny’s dog-vs-cat transfer / random PDF (shared in Discord; add to the final zip / Canvas upload if it’s not already in the repo)
...
# Put dog-vs-cat data where Jenny’s transfer preprocess expects it
```
...
python3 train_transfer.py     # Jenny’s transfer / random training
...
- Jenny will fill in / attach more for the transfer + random writeup on their side

That’s basically everything we did for P6. Thanks Jenny for the transfer side and for catching the PR pull thing early 🙂
